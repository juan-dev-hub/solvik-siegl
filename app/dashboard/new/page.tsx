...
export default function NewCertPage() {
  ...
  const validate = (f: File) => {
    ...
    if (!allowed.includes(f.type)) { setError(t.new_cert.file_type_error); return false }
    if (f.size > 5 * 1024 * 1024) { setError(t.new_cert.file_size_error); return false }
    setError(null)
    return true
  }
  ...
  const handleSubmit = async () => {
    if (!file || !issuedTo.trim()) { setError(t.new_cert.fields_error); return }
    setError(null)
    ...
    try {
      ...
      if (!res.ok || data.error) { setError(data.error ?? 'Error'); setStep('error'); return }
  ...
  const reset = () => {
    ...
    setExpiresAt(''); setStep('idle'); setError(null); setResult(null)
  }
